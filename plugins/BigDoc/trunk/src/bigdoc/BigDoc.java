...
public class BigDoc extends JPanel {
    ...
    private void initGUI() {
        ...
        editor.setStatusBar(status);

        // editor.setTokenMarker( new XMLTokenMarker() );
        editor.setCaretBlinkEnabled( jEdit.getBooleanProperty( "view.caretBlink" ) );
        editor.setBackground( jEdit.getColorProperty( "view.bgColor" ) );
        editor.setForeground( jEdit.getColorProperty( "view.fgColor" ) );

        TextAreaPainter painter = editor.getPainter();
        painter.setFont( jEdit.getFontProperty( "view.font" ) );
        painter.setBlockCaretEnabled( jEdit.getBooleanProperty( "view.blockCaret", false ) );
        painter.setBracketHighlightEnabled( jEdit.getBooleanProperty( "view.structureHighlight", true ) );
        painter.setBracketHighlightColor( jEdit.getColorProperty( "view.structureHighlightColor" ) );
        painter.setCaretColor( jEdit.getColorProperty( "view.caretColor" ) );
        painter.setEOLMarkersPainted( jEdit.getBooleanProperty( "view.eolMarkers", false ) );
        painter.setEOLMarkerColor( jEdit.getColorProperty( "view.eolMarkerColor" ) );
        painter.setLineHighlightEnabled( jEdit.getBooleanProperty( "view.lineHighlight", true ) );
    ...
    public void open( String filename ) {
        ...
        try {
            ...
            editor.getPainter().setStyles(SyntaxUtilities.getDefaultSyntaxStyles());
            editor.setCaretPosition( 0 );
            editor.scrollToCaret();
            
            // No: can I use the jEdit token marker classes instead? No, the current
            // jEdit TokenMarker is not at all the same as the TokenMarker in the bigdoc
            // package
