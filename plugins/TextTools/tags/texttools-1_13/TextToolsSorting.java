...
public class TextToolsSorting
{
  ...
  {
		if (!TextToolsPlugin.isTextAreaEditable(null, textArea)) return;
		/*
  ...
  {
		if (!TextToolsPlugin.isTextAreaEditable(view, textArea)) return;
...
//  ==> get text of all selected lines !!
	...
	static String getSubstringOrSpaces(String s, int beginIndex, int endIndex) {
		...
		if (actLen <= beginIndex) return MiscUtilities.createWhiteSpace(reqLen, 0);
		if (actLen < endIndex) 
			return s.substring(beginIndex) 
			+ MiscUtilities.createWhiteSpace(reqLen-actLen, 0);
