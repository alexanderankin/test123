...
//{{{ protected static void doBlockAction
	...
	if (overwriteBlock && lineEndOffset > 0) {
	  ...
	  int removeDecrement = insertPos + prefixString.length() + sourceString.length() -
		(lineEndOffset-1);
	  if (removeDecrement < 0) removeDecrement = 0;
	...
	if (isWithTabs) {
	  // change back to tabs
	  bsu.selectLine(selLines[i]);
	  bsu.JEditTextArea_spacesToTabs(); // Note: as long as there are bugs in textArea.spacesToTabs, a local
	  // patch is used
	}
	/*********************************************************
	 * memorize selection
	...
	if (newBeginSelection > buffer.getLength()) newBeginSelection = buffer.getLength();
	int newEndSelection = insertPos+prefixString.length() + sourceString.length();
	if (newEndSelection > buffer.getLength()) newEndSelection = buffer.getLength();
...
//				sourceString.length()));
	if (increment) currentIncValue += incrementValue;
      }
      /*********************************************************
