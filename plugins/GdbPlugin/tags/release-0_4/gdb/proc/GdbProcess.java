...
public class GdbProcess {
	...
	public void pause() {
		...
		if (progPid.size() == 1) {
			...
			// otherwise, look for the program as a gdb child
			//System.err.println("Multiple program processes are running");
			HashMap<Integer, Integer> gdbPid = findProcessId(procs, jEdit.getProperty(GeneralOptionPane.GDB_PATH_PROP));
			// if a single gdb process exists, look for its child
			if (gdbPid.size() == 1) {
				int gdbid = gdbPid.keySet().iterator().next().intValue();
				...
				if (jeditPid.size() == 1) {
					...
					int gdbid = findProcChild(jeditpid, gdbPid);
