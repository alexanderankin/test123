...
public class BufferFoldStringsDialog extends EnhancedDialog
{
	...
	public BufferFoldStringsDialog(View parent)
	{
		...
		if(foldStrings == null)
		{
			...
			strings = plugin.getDefaultModeFoldStrings(
				view.getBuffer().getMode().getName());
	...
	public void ok()
	{
		...
		{
			FoldStrings newStrings = new FoldStrings(startField.getText(),
				endField.getText(), useRegex.isSelected());
			
			if (newStrings.doFolding())
			{
				ConfigurableFoldHandlerPlugin.getInstance()
					.setBufferFoldStrings(view.getBuffer(), newStrings);
			}
			else
			{
				JOptionPane.showMessageDialog(view, jEdit.getProperty(
					"configurablefoldhandler.badstrings") + "\n" +
					newStrings.getErrorMessage(), jEdit.getProperty(
					"configurablefoldhandler.errortitle"),
