...
public class JavaCompletionFinder {
    ...
    public JavaCompletion complete( EditPane editPane, int caret ) {
        ...
        System.out.println( "+++++ word at cursor = " + word );
    ...
    private String getWordAtCursor( Buffer buffer ) {
        System.out.println( "+++++ in getWordAtCursor" );
        if ( caret <= 0 ) {
            System.out.println("+++++ caret is less than 0");
            return "";
        }
        if ( data == null ) {
        ...
        if ( caret - start < 0 ) {
            System.out.println("+++++ caret - start is less than 0");
            return "";
        }
        ...
        System.out.println("+++++ text = " + text + ", start = " + start + ", length = " + (caret - start));
        ...
        System.out.println("+++++ wordAtCursor returning: " + text);
        return text;
    }
    ...
    private JavaCompletion getPossibleCompletions( String word ) {
        ...
        if ( lastChar == '(' ) {
            ...
            System.out.println( "+++++ _word = " + _word );
            Class c = null;
                ...
                case TigerNode.FIELD:
                    for ( TigerNode var : child.getChildren() ) {
                        if ( var.getOrdinal() == TigerNode.VARIABLE ) {
                            VariableDeclarator vn = ( VariableDeclarator )var;
                            members.add( new JavaCompletionCandidate( vn.getName() + " : " + child.getType(), TigerLabeler.getFieldIcon() ) );
                        }
                    }
                    break;
                case TigerNode.ENUM:
            ...
            while ( c != null ) {
                ...
                for ( int i = 0; i < methods.length; i++ ) {
                    ...
                    if ( filter == null || methods[i].getName().startsWith( filter ) ) {
                        ...
                        params.append( ')' );
                        list.add( new JavaCompletionCandidate( method.getName() + params.toString() + " : " + method.getReturnType().getSimpleName(),
                ...
                for ( int i = 0; i < fields.length; i++ ) {
                    ...
                    if ( filter == null || fields[i].getName().startsWith( filter ) ) {
                        list.add( new JavaCompletionCandidate( fields[i].getName() + " : " + fields[i].getType().getSimpleName(),
