...
public class KappaLayout implements LayoutManager2, Serializable {
    ...
    public void layoutContainer(Container parent) {
        synchronized (parent.getTreeLock()) {
            ...
            for (int i = 0; i < _col_count; i++) {
                ...
                y = insets.top;
                if (i > 0) {
                    x += java.lang.Math.abs(_col_widths[ i - 1]);
                ...
                for (int j = 0; j < _row_count; j++) {
                    cell.y = j;
                    if (j > 0) {
                        y += java.lang.Math.abs(_row_heights[ j - 1]);
                    ...
                    if (c != null && c.isVisible()) {
                        ...
                        if (q.w == R) {
                            for (int n = i; n < _col_count; n++) {
                                sum_cols += java.lang.Math.abs(_col_widths[ n]);
                            }
                        }
                        else {
                        ...
                        if (q.h == R) {
                            for (int n = i; n < _row_count; n++) {
                                sum_rows += java.lang.Math.abs(_row_heights[ n]);
                            }
                        }
                        else {
    ...
    protected void calculateDimensions() {
        ...
        if (_col_widths != null) {
            ...
            System.arraycopy(temp, 0, _col_widths, 0, java.lang.Math.min(temp.length, _col_widths.length));
        }
        else {
            _col_widths = new int[ _col_count];
        }
        if (_row_heights != null) {
            ...
            System.arraycopy(temp, 0, _row_heights, 0, java.lang.Math.min(temp.length, _row_heights.length));
        }
        else {
            _row_heights = new int[ _row_count];
        }
        ...
        for (int i = 0; i < _col_count; i++) {
            for (int j = 0; j < _row_count; j++) {
                ...
                col_width = java.lang.Math.max(p.width, col_width);
            }
            ...
            if (_col_widths[ i] >= 0) {
                ...
                _preferred_width += java.lang.Math.abs(_col_widths[ i]);
            }
            col_width = 0;
        }
            ...
            while (en.hasMoreElements()) {
                ...
                for (int i = 0; i < same.length; i++) {
                    if (same[ i] < _col_widths.length) {
                        widest = java.lang.Math.max(widest, _col_widths[ same[ i]]);
                    }
                }
        ...
        for (int j = 0; j < _row_count; j++) {
            for (int i = 0; i < _col_count; i++) {
                ...
                row_height = java.lang.Math.max(p.height, row_height);
            }
            ...
            if (_row_heights[ j] >= 0) {
                ...
                _preferred_height += java.lang.Math.abs(_row_heights[ j]);
            }
            row_height = 0;
        }
            ...
            while (en.hasMoreElements()) {
                ...
                for (int i = 0; i < same.length; i++) {
                    if (same[ i] < _row_heights.length) {
                        tallest = java.lang.Math.max(tallest, _row_heights[ same[ i]]);
                    }
                }
    ...
    private void countColumns() {
        ...
        while (en.hasMoreElements()) {
            ...
            for (int i = 0; i < row.size(); i++) {
                if (row.get(i)) {
                    _col_count = java.lang.Math.max(_col_count, i + 1);
                }                // add 1 as column index is 0-based
            }
        }
    }
    ...
    private void countRows() {
        ...
        while (en.hasMoreElements()) {
            ...
            for (int i = 0; i < col.size(); i++) {
                if (col.get(i)) {
                    _row_count = java.lang.Math.max(_row_count, i + 1);
                }
            }
        }
    }
    ...
    public static class Constraints extends Object implements Cloneable, Serializable {
        ...
        public String toString() {
            StringBuffer sb = new StringBuffer();
            sb.append(String.valueOf(x) + ",") .append(String.valueOf(y) + ",");
            if (w == R) {
                sb.append("R,");
            }
            else {
            ...
            sb.append(String.valueOf(a) + ",") .append(String.valueOf(s) + ",") .append(String.valueOf(p)) .toString();
            return sb.toString();
        }
   ...
   public Component getDebugPanel() {
      java.awt.Panel p = new java.awt.Panel() {
         public Dimension getPreferredSize() {
            int width = 0;
            for ( int i = 0; i < _col_widths.length; i++ ) {
               width += java.lang.Math.abs(_col_widths[i]);
            }

            int height = 0;
            for ( int i = 0; i < _row_heights.length; i++ ) {
               height += java.lang.Math.abs(_row_heights[i]);
            }
         ...
         public void paint(Graphics g) {
            ...
            for ( int i = 0; i < _col_widths.length; i++ ) {
               x = java.lang.Math.abs(_col_widths[i]);
               g.drawLine(x, 0, x, d.height);
            }
            ...
            for ( int i = 0; i < _row_heights.length; i++ ) {
               y = java.lang.Math.abs(_row_heights[i]);
               g.drawLine(0, y, d.width, y);
            }
