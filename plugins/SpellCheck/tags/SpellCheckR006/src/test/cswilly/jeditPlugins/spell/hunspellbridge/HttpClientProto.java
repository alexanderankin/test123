...
public class HttpClientProto{
	...
	static void testModifDate(){
		try{
			...
			connect.connect();
			/* Same content-length as plain GET, so why bother ?
			HttpURLConnection connect = (HttpURLConnection)available_url.openConnection();
			connect.setRequestMethod("HEAD");
			connect.connect();
			System.out.println("content-length : "+connect.getContentLength());
			*/
			//Map<String,List<String>> heads = connect.getHeaderFields();
	...
	static void testIfModifiedSince(){
		try{
			...
			connect.connect();
			/* here we get 304 (not modified) and content-length is -1 */
			...
			connect.setIfModifiedSince(c.getTimeInMillis());
			connect.connect();
			/* here we get a 200 OK code, and size reported is 6070 */
			System.out.println("return code is :"+connect.getResponseCode());
			System.out.println("content length is:"+connect.getContentLength());
			//Map<String,List<String>> heads = connect.getHeaderFields();
			//System.out.println("Headers : "+heads);
		...
		try{
			...
			boolean copied = IOUtilities.copyStream(
				new ProgressObserver(){
					public void setMaximum(long v){System.out.println("max:"+v);}
					public void setStatus(String s){System.out.println("status:"+s);}
					public void setValue(long v){System.out.println("val:"+v);}
				},
				is,
				os,
				true
	...
	public static void testFetchUnzip(){
		...
		assert(dictArchname.endsWith(".zip"));//to remember to test this 
