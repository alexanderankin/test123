...
public class CharacterMap extends JPanel
{
	...
	/** Number of rows in the glyph table */
	...
	public CharacterMap(View view)
	{
		...
		font = view.getTextArea().getPainter().getFont();
		...
		encodingBox.add(Box.createHorizontalStrut(12));
		...
		float largeSize = (float) jEdit.getIntegerProperty("options.character-map.large-size", 36);
		largeFont = font.deriveFont(largeSize);
		...
		float superSize = (float) jEdit.getIntegerProperty("options.character-map.super-size", 128);
		Font superFont = font.deriveFont(superSize);
		superChar = new AntiAliasingLabel(superFont, " ");
		superChar.setBorder(BorderFactory.createLineBorder(Color.black));

		if (showLarge) {
			JLabel charCaption = new JLabel("Char: ");
		...
		table.setDefaultRenderer(Object.class, new AntiAliasingRenderer(font));
	 ...
	 */
	public void setStatusText(String ch, int row, int column)
	{
		int num;

		int n = row * 16 + column + getBlockOffset();
		...
		if (ch == null) {
			...
			if(!encoding.equals("UTF8") || n < 128)
			{
				buf.append(" Int: ").append(n);
				buf.append(" Hex: ");
			...
			else if (n < 2048)
			{
				...
				buf.append(" Hex: ");
				buf.append(Integer.toString(b1, 16));
				buf.append("-");
				buf.append(Integer.toString(b2, 16));
			}
			else
			{
				int b1 = 224;
				...
				buf.append(Integer.toString(b2, 16));
				buf.append("-");
				buf.append(Integer.toString(b3, 16));
			}
	...
	{
		return encoding.equals("Unicode") || encoding.startsWith("UTF");
	}
	...
	class MouseHandler extends MouseInputAdapter
	{
		...
		private void displaySuperChar(String ch, int row, int column)
		{
			...
			if (offsetSuper) {
				...
				GraphicsConfiguration gf = thisPanel.getGraphicsConfiguration();
	...
	class AntiAliasingLabel extends JLabel
	{
		...
		/**
		 * Construct an empty label with the given font
		 *
		 * @param  font  Font to render label value in
		 */
		public AntiAliasingLabel(Font font)
		{
			this(font, "");
		}

		/**
		 * Construct a label with the given text using the default font
		 *
		 * @param  text  Value of label
		 */
	...
	{
		/**
		 * Construct a renderer whose label contents should be 
		 * drawn in the given font
		 * @param font Font to use when rendering label contents
		 */
		public AntiAliasingRenderer(Font font)
		{
			super(font);
		}
