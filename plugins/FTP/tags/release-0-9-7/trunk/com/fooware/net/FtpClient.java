...
/**
 * Most of the method descriptions have been copied from RFC959, authors J.
 * Postel and J. Reynolds, 1985. The RFC can be viewed in its entirety at <A
 * HREF="http://sunsite.auc.dk/RFC/rfc/rfc959.html">http://sunsite.auc.dk/RFC/rfc/rfc959.html</A>.
 * <P>
 * Click <A HREF="ftp_snippets.html">here</A> to see some snippets that
 * demonstrate some the common methods.
 * <P>
 * Some have reported bugs when sending/receiving binary files via the
 * <CODE>FtpReader</CODE> and <CODE>FtpWriter</CODE>. To be safe, it is
 * recommended to use the analogous <CODE>FtpInputStream</CODE> and
 * <CODE>FtpOutputStream</CODE> methods. E.g., rather than <CODE>list()</CODE>,
 * use <CODE>listStream()</CODE>.
 * 
...
public class FtpClient {
	 ...
	 * same response. New repsonses are added as requests are issued to the FTP
	 * server and when data connections are closed.
	 */
	...
	public void connect(String hostName, int port) throws IOException {
		if (proxy != null) {
			cmdSocket = proxy.openSocket(hostName, port);
		} else {
	 ...
	 * information. Transfer parameters are similarly unchanged. The argument is
	 * a pathname specifying a directory or other system dependent file group
	 * designator. <BR>
	 * <I>Per RFC959</I>
	 */
	 ...
	 * <I>Per RFC959</I><BR>
	 * <B>Note:</B> The preceeding excerpt is provided for informational
	 * purposes. For *this* ftp client, you must issue at least one dataPort()
	 * command is you plan to do any ftp processing that uses the data
	 * connection rather than the command connect, e.g. retrieve. Per the RFC,
	 * there is a default user port number, but I have yet to find the port
	...
	public void passive() throws IOException {
		...
		try {
			...
			if (proxy != null) 
				passiveSocket = proxy.openSocket(remoteAddr, port);
			else 
				passiveSocket = new Socket(remoteAddr, port);
			
			passiveSocket.setSoTimeout(TIMEOUT);
		} catch (StringIndexOutOfBoundsException e) {
			Log.log(Log.ERROR, this, Thread.currentThread() + ": Couldn't set passive, trying data port");
			dataPort();
			return;
		}
	}
	 ...
	 * parameters are separated by a <SP> (Space, ASCII code 32). The following
	 * codes are assigned for type: <BR>
	 * <CODE><PRE>
	 *              \    /               
	 *    A - ASCII |    | N - Non-print
	 ...
	 * <I>Per RFC959</I><BR>
	 * For convenience, the following constants can be used: <TABLE>
	 * <TR>
	 * <TD>
	 * <UL>
	 * <LI>ASCII_TYPE</LI>
	 ...
	 * following codes are assigned for structure: <BR>
	 ...
	 * following codes are assigned for transfer modes: <BR>
	 * <CODE><PRE>
	 *   S - Stream
	 *   B - Block
	 *   C - Compressed 
	 * </PRE></CODE> The default transfer mode is Stream. <BR>
	 * <I>Per RFC959</I><BR>
	 * For convenience, the following constants can be used: <TABLE>
	 * <TR>
	 * <TD>
	 ...
	 * <I>Per RFC959</I> <BR>
	 * <B>Note:</B> It is important that the reader be closed after it is done
	 * being processed. This signals the ftp client to get the next response
	 * from the server.
	 * 
	 ...
	 * already exist. <BR>
	 * <I>Per RFC959</I> <BR>
	 * <B>Note:</B> It is important that the writer be closed after it is done
	 * being processed. This signals the ftp client to get the next response
	 * from the server.
	 * 
	 ...
	 * is still in progress. In the first case, the server closes the data
	 * connection (if it is open) and responds with a 226 reply, indicating that
	 * the abort command was successfully processed. <BR>
	 ...
	 * provided by the user-FTP process. <BR>
	 * <I>Per RFC959</I>
	 */
	 ...
	 * command is intended to return information that can be used by a program
	 * to further process the files automatically. For example, in the
	 * implementation of a "multiple get" function. <BR>
	 * <I>Per RFC959</I> <BR>
	 * <B>Note:</B> It is important that the reader be closed after it is done
	 * being processed. This signals the ftp client to get the next response
	 * from the server.
	 ...
	 * listed in the current version of the Assigned Numbers document [4]. <BR>
	 * <I>Per RFC959</I>
	 */
