/*:folding=indent:
 * ProjectMacros.java - Macros for working with project files.
...
public class ProjectMacros
{
	...
	public static int getBufferWordCount(Buffer buff)
	{
		...
		{
			System.err
				.println("Regular Expression Failed!! Check expression in "
	...
	public static String getMainTeXPath(Buffer buffer)
	{
		...
		REMatch[] match = TextMacros.findInDocument(buffer, regex.toString(), 0, Math.min(
			buffer.getLineCount() - 1, 5));
	...
	public static int getProjectWordCount(View view, Buffer buffer)
	{
		...
		for (Enumeration it = files.preorderEnumeration(); it.hasMoreElements();)
		{
			...
			Buffer buff = jEdit
				.openTemporary(view, in.getParent(), in.getName(), false);
	...
	public static boolean isTeXFile(Buffer b)
	{
		...
		// Init: Notify SideKick that we shall parse this buffer if no
		// parser set yet
		String modeName = null;
		for (int i = 0; i < texModes.length; i++)
		{
			modeName = texModes[i].trim();
			// Note: buffer.getProperty(key) tries 1st
			// jEdit.getProperty("mode."+mode+"."+key);
			// (and the property's value is cached)
			String parserName = jEdit.getProperty("mode." + modeName
				+ ".sidekick.parser");
		...
		for (int i = 0; i < texModes.length; i++)
		{
			...
			// "LaTeXTools", "LaTeXTools: comparing '"+s+"' and
			// '"+texModes[i]+"'.");
	...
	public static void renameLabel(View view, Buffer buffer, String oldName, String newName)
	{
		...
		RE oldLabel = null;
		StringBuffer find = new StringBuffer("((?:\\\\ref)|(?:\\\\label))(\\{)").append(
			oldName).append("(\\})");
		...
		for (Enumeration en = files.depthFirstEnumeration(); en.hasMoreElements();)
		{
			...
			boolean found = SearchAndReplace.replace(view, buff, 0,
				buff.getLength() - 1);
	...
	private static File[] getImportsInRange(Buffer buffer, int startLine, int endLine)
	{
		...
		REMatch[] matches = TextMacros.findInDocument(buffer, IMPORT_REG_EX, startLine,
			endLine);
		...
		for (int i = 0; i < matches.length; i++)
		{
			...
			String preText = buffer.getLineText(line).substring(0,
				(posn - buffer.getLineStartOffset(line)) + 1);
	...
	private static DefaultMutableTreeNode getNestedImports(View view, File in)
	{
		...
		outer: for (int i = 0; i < children.length; i++)
		{
			...
			if (!f.exists())
			{
				...
				StringTokenizer str = new StringTokenizer(jEdit.getProperty(
	...
	private static void _showInformation(View view, Buffer buffer)
	{
		...
		dockable.setInfoPanel(new JLabel(
			"<html><font color='#dd0000'>Getting information..."),
			"Project Information:");
		...
		JSplitPane panel = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true,
			new JScrollPane(editor), new JScrollPane(proj));
		JComponent wind = view.getDockableWindowManager().getDockable(
