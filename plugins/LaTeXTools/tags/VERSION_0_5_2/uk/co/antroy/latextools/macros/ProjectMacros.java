...
public class ProjectMacros {
    ...
    public static int getBufferWordCount(Buffer buff) {
        ...
        try {
            ...
            System.err.println(
                        "Regular Expression Failed!! Check expression in " + 
    ...
    public static String getMainTeXPath(Buffer buffer) {
        ...
        REMatch[] match = TextMacros.findInDocument(buffer, regex.toString(), 
                                                    0, 
                                                    Math.min(buffer.getLineCount() - 1, 
                                                             5));
    ...
    public static void openAllProjectFiles(View view, Buffer buffer) {
        ...
        for (Enumeration en = files.depthFirstEnumeration();
             en.hasMoreElements();) {
    ...
    public static void renameLabel(View view, Buffer buffer, String oldName, 
                                   String newName) {
        ...
        StringBuffer find = new StringBuffer(
                                        "((?:\\\\ref)|(?:\\\\label))(\\{)").append(
                                        oldName).append("(\\})");
        StringBuffer replace = new StringBuffer("$1$2").append(newName).append(
                                           "$3");
        ...
        for (Enumeration en = files.depthFirstEnumeration();
            ...
            DefaultMutableTreeNode node = (DefaultMutableTreeNode)en.nextElement();
            File f = (File)node.getUserObject();
            Buffer buff = jEdit.openTemporary(view, f.getParent(), f.getName(), 
                                              false);
    ...
    private static File[] getImportsInRange(Buffer buffer, int startLine, 
                                            int endLine) {

        REMatch[] matches = TextMacros.findInDocument(buffer, IMPORT_REG_EX, 
                                                      startLine, endLine);
        ...
        for (int i = 0; i < matches.length; i++) {
            ...
            String preText = buffer.getLineText(line).substring(0, 
                                                                (posn - buffer.getLineStartOffset(
                                                                                    line)) + 1);
...
outer: 
    ...
    private static void _showInformation(View view, Buffer buffer) {
        ...
        dockable.setInfoPanel(new JLabel("<html><font color='#dd0000'>Getting information..."), 
                              "Project Information:");
    ...
    private static class LaTeXMutableTreeNode
