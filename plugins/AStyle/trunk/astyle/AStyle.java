...
public class AStyle implements ASResource {
            ...
            for (int i = 0; i < fileNameVector.size(); i++) {
                ...
                if (!origFile.renameTo(inFile)) {
                    error("Could not rename",
                          origFile.toString() + " to " + inFile.toString());
                }
    ...
    private static boolean parseOptions(ASFormatter formatter, Vector options, String errorInfo) {
        boolean ok = true;
        String subArg = "";
        String arg;
    ...
    private static boolean parseOption(ASFormatter formatter, String arg, String errorInfo) {
        if (IS_PARAM_OPTION(arg, "suffix=")) {
            ...
            formatter.setMinConditionalIndentLength(GET_NUM_PARAM(minIndentParam, 0));
        }
        else if (IS_PARAM_OPTIONS(arg, "M", "max-instatement-indent=")) {
            String maxIndentParam = GET_PARAMS(arg, "M", "max-instatement-indent=");
            formatter.setMaxInStatementIndentLength(GET_NUM_PARAM(maxIndentParam, 40));
        }
    ...
    private static void importOptions(Reader in, Vector optionsVector)
            throws IOException
    {
    ...
    private static final String HELP =
        ...
        + "       system it is: " + System.getProperty("user.home") + "\n"
