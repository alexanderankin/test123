...
public class ASFormatter extends ASBeautifier {
    ...
    /**
     * initialize the ASFormatter. This method should be called every time
     * a ASFormatter object is to start formatting a new source file.
     ...
     * <ul>
     *   <li><code>true</code>
     *      brackets just before closing headers (e.g. 'else', 'catch')
     *      will be broken, even if standard brackets are attached.</li>
     *   <li><code>false</code>
     *      closing header brackets will be treated as standard brackets.</li>
     * </ul>
    ...
    /**
     * get 'else if()' breaking mode.
     * If true, 'else' headers will be broken from their succeeding 'if' headers,
    ...
    /**
     * get operator padding mode.
     * If true, statement operators will be padded with spaces around them,
    ...
    /**
     * get parenthesis padding mode.
     * If true, statement parenthesis will be padded with spaces around them,
    ...
    /**
     * <p>get option to break closing header blocks of code (such as 'else',
    ...
    /**
     * <p>set option to break closing header blocks of code (such as 'else',
     * 'catch', ...) with empty lines.</p>
     * @param  state  true = convert, false = don't convert.
     */
        ...
        while (!isLineReady) {
            if (shouldReparseCurrentChar) {
                ...
                return beautify(readyFormattedLine);
                 ...
                 // make sure that a virgin '{' at the begining of the file
            ...
            if (isImmediatelyPostHeader) {
                ...
                if (shouldBreakOneLineStatements) {
                    ...
                    if (shouldBreakElseIfs) {
                        ...
                        String upcomingHeader = findHeader(headers);
    ...
    /**
     * jump over the leading white space in the current line, IF the line
    ...
    /**
     * check if the currently reached open-bracket (i.e. '{') opens
     * a definition type block (such as a class or namespace),
     * a command block (such as a method block) or
     * a static array.
     *
     * This method takes for granted that the current character is an
     * opening bracket.
     *
     ...
     * pointer- or reference symbol, or another operator.
    ...
    /**
     * check if the currently reached '-' character is a urinary minus.
     * This method takes for granted that the current character is a '-'.
    ...
    /**
     * <p>check if the currently reached '-' or '+' character is part of an
     * exponent, i.e. 0.2E-5.</p>
     *
    ...
    /**
     * <p>check if a one-line bracket has been reached, i.e. if the
     * currently reached '{' character is closed with a complimentry '}'
     * elsewhere on the current line.</p>
     *
     ...
     * line-break has been registered, and if so break the formatted line,
     * and only then append the character into the next formatted line.
     *
     ...
     * Unless disabled (via canBreakLine == false), first check if a
     * line-break has been registered, and if so break the formatted line,
     * and only then append the sequence into the next formatted line.
     *
    ...
    /**
     * append a space to the current formattedline, UNLESS the last
     * character is already a white-space character.
     */
    ...
    /**
     * check if one of a set of headers has been reached in the current
