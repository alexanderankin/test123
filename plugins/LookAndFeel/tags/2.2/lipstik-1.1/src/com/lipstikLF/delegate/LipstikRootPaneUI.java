...
public class LipstikRootPaneUI extends BasicRootPaneUI
{
	/**
	 * The amount of space (in pixels) that the cursor is changed on.
	 */
	private static final int CORNER_DRAG_WIDTH = 15;
	...
	void installBorder(JRootPane root)
	{
		...
		if (style == JRootPane.NONE)
			...
			LookAndFeel.installBorder(root, "InternalFrame.border"); 
	}
	...
	private void installLayout(JRootPane root)
	{
		if (layoutManager == null)
			layoutManager = new MetalRootLayout();
	...
	{
		installBorder(root);
		JComponent titlePane = new LipstikTitlePane(root, this);
	...
	private void setTitlePane(JRootPane root, JComponent titlePane)
	{
		...
		if (oldTitlePane != null)
		{
			...
			if (oldTitlePane instanceof LipstikTitlePane)
				((LipstikTitlePane)oldTitlePane).uninstall();			
		}
		if (titlePane != null)
		{
	...
	private static class MetalRootLayout implements LayoutManager2
	{
		...
		public Dimension preferredLayoutSize(Container parent)
		{
			...
			if (root.getWindowDecorationStyle() != JRootPane.NONE
					&& (root.getUI() instanceof LipstikRootPaneUI))
			{
				JComponent titlePane = ((LipstikRootPaneUI) root.getUI()).getTitlePane();
				if (titlePane != null)
				{
			...
			return new Dimension(Math.max(Math.max(cpWidth, mbWidth), tpWidth)
					+ i.left + i.right, cpHeight + mbHeight + tpWidth + i.top + i.bottom);
		}
		...
		public Dimension maximumLayoutSize(Container target)
		{
			...
			if (root.getWindowDecorationStyle() != JRootPane.NONE && 
					(root.getUI() instanceof LipstikRootPaneUI))
			{
				JComponent titlePane = ((LipstikRootPaneUI) root.getUI())
						.getTitlePane();
				if (titlePane != null)
				{
	...
	private class MouseInputHandler implements MouseInputListener
	{
		...
		public void mousePressed(MouseEvent ev)
		{
			...
			if (getTitlePane() != null && getTitlePane().contains(convertedDragWindowOffset) && dragCursor==0)
			{
		...
		{
			if (isMovingWindow != null && window != null && !window.isValid())
			{
		...
		public void mouseMoved(MouseEvent ev)
		{
			...
			if (root.getWindowDecorationStyle() == JRootPane.NONE || isMovingWindow != null)
				return;
		...
		private void adjust(Rectangle bounds, Dimension min, 
					int deltaX, 
					int deltaY, 
					int deltaWidth,
					int deltaHeight)
		...
		{
			if (isMovingWindow == null) return;

			Window w = (Window) ev.getSource();
			Point pt = ev.getPoint();

			if (isMovingWindow.booleanValue())
			{
