...
public class ActivationPanel extends JPanel implements ActionListener, MouseListener {
    ...
    public void actionPerformed( ActionEvent e ) {
        ...
        if ( e.getSource() == unload ) {
            ...
            sw.stop();
            jEdit.getActiveView().getStatus().setMessage ( plugin + " " + jEdit.getProperty( "activator.unloaded_in", "unloaded in" ) + " " + sw );
        }
        if ( e.getSource() == activate ) {
            sw.start();
            PluginManager.getInstance().activate(plugin);
            sw.stop();
            jEdit.getActiveView().getStatus().setMessage ( plugin + " " + jEdit.getProperty( "activator.activated_in", "activated in" ) + " " + sw );
        }
        if ( e.getSource() == deactivate ) {
            sw.start();
            PluginManager.getInstance().deactivate(plugin);
            sw.stop();
            jEdit.getActiveView().getStatus().setMessage ( plugin + " " + jEdit.getProperty( "activator.deactivated_in", "deactivated in" ) + " " + sw );
        }
    ...
    public void mouseReleased( MouseEvent unused ) {
        ...
        if ( row >= 0 ) {
            ...
            plugin = ( Plugin ) table.getValueAt( row, 0 );
            if ( plugin == null ) {
                return;
            }
            ...
            if ( plugin.getStatus() == ERROR || plugin.isLibrary() ) {
                activate.setEnabled( false );
            }
        } else {
    ...
    class ActivationTableModel extends DefaultTableModel implements Observer {
        ...
        public Object getValueAt( int row, int col ) {
            Plugin p = PluginList.getInstance().get( row );
            switch ( col ) {
                case 0:
                    return p;
                case 1:
                    int status = PluginManager.getStatus( p.getJAR() );
                    return PluginManager.getStatusText( status );
                case 2:
                    return p.canLoadOnStartup() && p.isLoadOnStartup();
                default:
                    return jEdit.getProperty( "activator.error", "error" );
            }
        }
    ...
    class StringRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent( JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column ) {
            ...
            if ( column == 1 ) {
                ...
                if ( value == LOADED ) {
                    displayName.append( "<html><font color=yellow>&#9830;</font> " ).append( name );
                } else if ( value == ACTIVATED ) {
                    displayName.append( "<html><font color=green>&#9830;</font> " ).append( name );
                } else if ( value == PluginList.ERROR ) {
                    displayName.append( "<html><font color=red>&#9830;</font> " ).append( name );
                }
