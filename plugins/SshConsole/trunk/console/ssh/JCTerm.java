...
public class JCTerm implements Term, KeyListener
{
	...
	private final Object[] colors = { Color.black, Color.red, Color.green, Color.yellow,
	...
	public JCTerm(Console console, ConsolePane consolePane)
	{
		 ...
		 * foog=(Graphics2D)(background.getGraphics());
		 * foog.setColor(getBackGround()); foog.fillRect(0, 0,
		 * char_width, char_height); foog.dispose(); }
		 */
		setSize(getTermWidth(), getTermHeight());

		/*
		 * if(splash!=null) splash.draw(img, getTermWidth(),
		 * getTermHeight()); else clear();
		 */
	...
	public void setSize(int w, int h)
	{
		...
		/*
		 * if(imgOrg!=null){ Shape clip=graphics.getClip();
	...
	{
		// TODO : we need to calculate the caret position when giving an x,y coord?
	...
	{
		// System.out.println("scroll_area: "+x+" "+y+" "+w+" "+h+"
		// "+dx+" "+dy);
	...
	public void drawBytes(byte[] buf, int s, int len, int x, int y)
	{
		...
		// System.out.println("drawString: "+x+","+y+" "+len+" "+new
		// String(buf, s, len));
